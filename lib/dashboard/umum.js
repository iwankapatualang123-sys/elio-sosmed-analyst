...
export async function loadUmum(supabase, { month = null, cat = null } = {}) {
  ...
  for (const b of bundles) {
    ...
    // Peta per-tanggal utk fallback (link pendek vt.tiktok.com tak punya video_id):
    // kalau di tanggal rencana HANYA ADA 1 konten platform itu, cocokkan by tanggal.
    const ttByDate = new Map();
    for (const c of b.ttContent) { const d = String(c.post_date || "").slice(0, 10); if (!d) continue; if (!ttByDate.has(d)) ttByDate.set(d, []); ttByDate.get(d).push(c); }
    const igByDate = new Map();
    for (const c of b.igContent) { const d = String(c.published_at || "").slice(0, 10); if (!d) continue; if (!igByDate.has(d)) igByDate.set(d, []); igByDate.get(d).push(c); }
    ...
    for (const p of b.plans) {
      ...
      let matched = false;
      if (ttMatch) { push(Number(ttMatch.total_views) || 0, ttEr(ttMatch)); matched = true; }
      if (igMatch) { push(Number(igMatch.views) || 0, erOf(igMatch)); matched = true; }
      // Fallback aman: link tak cocok (mis. link pendek) → cocokkan by tanggal HANYA
      // bila di tanggal itu tepat 1 konten platform terkait & belum dipakai plan lain.
      if (!matched && p.post_date) {
        const d = String(p.post_date).slice(0, 10);
        const plats = Array.isArray(p.platforms) ? p.platforms : [];
        const ttCands = (ttByDate.get(d) || []).filter((c) => !usedTt.has(c.video_id));
        const igCands = (igByDate.get(d) || []).filter((c) => !usedIg.has(c.post_id));
        if (ttCands.length === 1 && (!plats.length || plats.includes("tiktok"))) {
          usedTt.add(ttCands[0].video_id); push(Number(ttCands[0].total_views) || 0, ttEr(ttCands[0]));
        } else if (igCands.length === 1 && (!plats.length || plats.includes("instagram"))) {
          usedIg.add(igCands[0].post_id); push(Number(igCands[0].views) || 0, erOf(igCands[0]));
        }
      }
    }
  }
