...
export async function loadUmum(supabase, { month = null, cat = null } = {}) {
  ...
  const bundles = await Promise.all(outlets.map(async (acc) => {
    const [{ data: ttc }, { data: igc }, { data: hist }, { data: snaps }, { data: igDaily }, { data: gender }, { data: terr }, { data: plans }, { data: bundleIgAud }] = await Promise.all([
      ...
      supabase.from("instagram_audience").select("snapshot_date, female_pct, male_pct").eq("tiktok_account_id", acc.id),
    ]);
    return {
      acc,
      ttContent: ttc || [],
