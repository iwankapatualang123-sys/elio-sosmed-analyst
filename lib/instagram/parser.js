...
const DAILY_METRICS = [
  { key: "views", match: /tayangan|views|impressions/i },
  { key: "reach", match: /jangkauan|reach/i },
  { key: "profile_visits", match: /kunjungan|profile visit/i },
  { key: "new_followers", match: /pengikut|follow/i },
  { key: "interactions", match: /interaksi|interaction/i },
];
...
export function parseDailyMetricCsv(text) {
  ...
  const headerIdx = rows.findIndex((r) => /^(tanggal|date)$/i.test(String(r[0] ?? "").trim()));
  if (headerIdx < 0) throw new Error('Header "Tanggal"/"Date" tidak ditemukan — bukan file metrik harian.');
...
// Header dikenali BILINGUAL (Indonesia + Inggris) karena ekspor Meta Business
// Suite mengikuti bahasa akun — Elio berbahasa Indonesia, Golio/Hokca dll bisa
// berbahasa Inggris. Regex di-anchor (^...$) agar "Komentar" tak tertukar
// "Komentar data" / "Comments" tak tertukar "Data comment".
const CONTENT_HEADERS = [
  { key: "post_id", match: /^(id postingan|post id)$/i },
  { key: "ig_account_id", match: /^(id akun|account id)$/i },
  { key: "username", match: /^(nama pengguna akun|account username)$/i },
  { key: "account_name", match: /^(nama akun|account name)$/i },
  { key: "description", match: /^(deskripsi|description)$/i },
  { key: "duration_s", match: /^(durasi|duration)/i },
  { key: "published_at", match: /^(waktu penerbitan|publish time)$/i },
  { key: "permalink", match: /^permalink$/i },
  { key: "post_type", match: /^(jenis postingan|post type)$/i },
  { key: "date_scope", match: /^(tanggal|date)$/i },
  { key: "views", match: /^(tayangan|views|plays)$/i },
  { key: "reach", match: /^(jangkauan|reach)$/i },
  { key: "likes", match: /^(suka|likes)$/i },
  { key: "shares", match: /^(frekuensi dibagikan|shares)$/i },
  { key: "comments", match: /^(komentar|comments)$/i },
  { key: "saves", match: /^(frekuensi disimpan|saves|saved)$/i },
  { key: "profile_visits", match: /^(kunjungan profil|profile visits)$/i },
  { key: "replies", match: /^(balasan|replies)$/i },
  { key: "navigation", match: /^(navigasi|navigation)$/i },
  { key: "sticker_taps", match: /^(ketukan stiker|sticker taps)$/i },
  { key: "follows", match: /^(mengikuti|follows)$/i },
];
...
export function parseContentCsv(text) {
  ...
  if (colFor.post_id === undefined || colFor.permalink === undefined) {
    throw new Error('Header "ID Postingan"/"Post ID" atau "Permalink" tidak ditemukan — bukan file per konten.');
  }
