...
// Kelompokkan baris berdasarkan kolom (mis. tiktok_account_id) -> Map<key, rows[]>.
// Dipakai agar data semua outlet bisa diambil dalam SATU query (.in) lalu dibagi
// per outlet di memori — jauh lebih ringan daripada 1 query per outlet (N+1).
...
export async function loadPortfolio(supabase, options = {}) {
  ...
  // di memori — menghindari N+1 (dulu 2 query per outlet).
  const list = accounts || [];
  const ids = list.map((a) => a.id);
  const [{ data: allContent }, { data: allHistory }] = await Promise.all([
    supabase.from("tiktok_content").select("tiktok_account_id, post_date, total_views, total_likes, total_comments, total_shares").in("tiktok_account_id", ids),
    supabase.from("tiktok_follower_history").select("tiktok_account_id, date, followers, diff_from_previous_day").in("tiktok_account_id", ids),
  ]);
  const contentBy = groupBy(allContent, "tiktok_account_id");
  const historyBy = groupBy(allHistory, "tiktok_account_id");
  const monthSet = new Set();
  const branches = list.map((acc) => {
    const content = contentBy.get(acc.id) || [];
    const history = historyBy.get(acc.id) || [];
...
export async function loadPortfolioInstagram(supabase, options = {}) {
  ...
  const list = accounts || [];
  const ids = list.map((a) => a.id);
  const [{ data: allContent }, { data: allDaily }] = await Promise.all([
    supabase.from("instagram_content").select("tiktok_account_id, published_at, views, likes, comments, shares, saves, follows, is_collab").in("tiktok_account_id", ids),
    supabase.from("instagram_daily_metrics").select("tiktok_account_id, metric, date, value").in("tiktok_account_id", ids),
  ]);
  const contentBy = groupBy(allContent, "tiktok_account_id");
  const dailyBy = groupBy(allDaily, "tiktok_account_id");
  const monthSet = new Set();
  const branches = list.map((acc) => {
    const content = contentBy.get(acc.id) || [];
    const daily = dailyBy.get(acc.id) || [];
    // Kumpulkan bulan yang tersedia (dari tanggal terbit konten & metrik harian) —
    // supaya bulan yang baru punya data IG saja tetap muncul di filter bulan.
    (content || []).forEach((r) => r.published_at && monthSet.add(monthOf(String(r.published_at).slice(0, 10))));
    (daily || []).forEach((r) => r.date && monthSet.add(monthOf(String(r.date).slice(0, 10))));
...
export async function loadBranchDetail(supabase, accountId, options = {}) {
  ...
  // Analisis Pertumbuhan: diagnosis SEBAB saat kenaikan follower melambat. Dua
  // bulan yang dibandingkan:
  //   - kalau 1 bulan dipilih  : bulan itu vs bulan sebelumnya.
  //   - kalau "Semua bulan"     : 2 bulan TERAKHIR yang punya data (supaya kartu
  //     tetap muncul & mudah ditemukan di tampilan default, bukan cuma saat difilter).
  // growthDiagnosis membawa curMonth/prevMonth-nya sendiri (tidak bergantung filter).
  let growthDiagnosis = null;
  {
    ...
    if (month) {
      curM = month;
      prevM = prevMonthOf(month);
    } else {
      const withData = [...new Set(historyRows.map((r) => monthOf(r.date)).filter(Boolean))].sort();
      if (withData.length >= 2) { curM = withData[withData.length - 1]; prevM = withData[withData.length - 2]; }
    }
    if (curM && prevM) {
      const curContent = contentRows.filter((r) => inMonth(r.post_date, curM));
      const prevContent2 = contentRows.filter((r) => inMonth(r.post_date, prevM));
      const curHistory = historyRows.filter((r) => inMonth(r.date, curM));
      const prevHistory2 = historyRows.filter((r) => inMonth(r.date, prevM));
      if (prevContent2.length > 0 || prevHistory2.length > 0) {
        const curCs = metrics.summarizeContent(curContent);
        const curGrowth = metrics.followerGrowth(curHistory);
        const prevCs2 = metrics.summarizeContent(prevContent2);
        const prevGrowth2 = metrics.followerGrowth(prevHistory2);
        const dg = diagnoseGrowthDrop({
          current: { netGrowth: curGrowth.netGrowth, totalVideos: curCs.totalVideos, engagementRate: curCs.engagementRateOverall },
          previous: {
            netGrowth: prevGrowth2.netGrowth,
            totalVideos: prevCs2.totalVideos,
            engagementRate: prevCs2.engagementRateOverall,
            videoViews: prevContent2.map((r) => Number(r.total_views) || 0),
          },
        });
        if (dg) growthDiagnosis = { ...dg, curMonth: curM, prevMonth: prevM };
      }
    }
  }
  ...
  // Metrik BULAN BERJALAN (kalender) — untuk pencapaian target bulanan saat filter
  // di "Semua bulan". Selalu dihitung, tak tergantung filter.
  const cmContent = contentRows.filter((r) => inMonth(r.post_date, currentMonth()));
  const cmHistory = historyRows.filter((r) => inMonth(r.date, currentMonth()));
  const thisMonth = { month: currentMonth(), summary: metrics.summarizeContent(cmContent), growth: metrics.followerGrowth(cmHistory) };
