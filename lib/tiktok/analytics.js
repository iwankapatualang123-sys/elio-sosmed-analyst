...
export async function loadBranchDetail(supabase, accountId) {
  ...
  return {
    summary: cs,
    growth: metrics.followerGrowth(history.data || []),
    history: (history.data || []).map((r) => ({ date: r.date, followers: r.followers })),
    viewers: metrics.viewersRatio(viewers.data || []),
    bestHours: metrics.bestPostingTimes(activity.data || [], { top: 5 }),
    gender: (gender.data || [])[0] || null,
    territories: (territories.data || []).slice(0, 6),
    topVideos: metrics.rankVideos(cs.videos, { by: "views", limit: 5 }),
  };
}
