...
// Fungsi: extractIgShortcode
// Ambil shortcode dari URL Instagram (/reel/<sc>, /reels/<sc>, /p/<sc>, /tv/<sc>).
// Dipakai untuk mencocokkan link tayang IG rencana ke permalink data report IG.
// Output: string shortcode, atau null kalau bukan URL post/reel IG.
export function extractIgShortcode(url) {
...
export function matchPlanStatusMulti(plan = {}, contents = [], options = {}) {
  ...
  const igContents = options.igContents || []; // data report Instagram (permalink dll)
  ...
  for (const p of platforms) {
    if (p === "tiktok") {
      ...
      if (link) {
        // Instagram: verifikasi via SHORTCODE permalink vs data report IG (kalau ada).
        // Threads (belum ada sumber data) tetap berbasis link -> Uploaded.
        if (p === "instagram") {
          const sc = extractIgShortcode(link);
          const match = sc ? igContents.find((c) => extractIgShortcode(c.permalink) === sc) : null;
          perPlatform[p] = match
            ? { status: PLAN_STATUS.VERIFIED, match, matchedBy: "ig-permalink", hint: null }
            : { status: PLAN_STATUS.UPLOADED, match: null, matchedBy: "link", hint: null };
        } else {
          perPlatform[p] = { status: PLAN_STATUS.UPLOADED, match: null, matchedBy: "link", hint: null };
        }
      }
