...
// Fungsi: heatmapPeak
// Cari sel PALING RAMAI di heatmap hari×jam (dipakai laporan bulanan: "hari & jam
// terbaik", bukan sekadar jam agregat). Input: heatmap { [weekday 0-6]: { [hour]: avg } }.
// Output: { weekday (0=Min..6=Sab), hour (0-23), value } atau null kalau kosong.
function heatmapPeak(heatmap) {
  let peak = null;
  for (const wd of Object.keys(heatmap || {})) {
    for (const h of Object.keys(heatmap[wd])) {
      const value = num(heatmap[wd][h]);
      if (peak === null || value > peak.value) peak = { weekday: Number(wd), hour: Number(h), value };
    }
  }
