...
// Input: nama cabang + array insight + periodLabel opsional (mis. "bulan April 2026",
// dipakai saat Dashboard sedang meninjau 1 bulan spesifik agar narasi AI menyebut
// periode yang benar, bukan menyiratkan "sepanjang masa"). Output: string prompt.
export function buildInsightPrompt(namaCabang, insights, periodLabel = null) {
  const lines = insights.map((i) => `- ${i.aspek}: ${i.kesimpulan} Saran: ${i.saran}`).join("\n");
  const periode = periodLabel ? ` untuk periode ${periodLabel}` : "";
  return [
    `Kamu analis media sosial. Rangkai poin data berikut untuk akun TikTok "${namaCabang}"${periode} `,
