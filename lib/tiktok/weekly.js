...
// — idealnya sudah difilter ke 1 bulan.
//
// PENTING (konsistensi data): netGrowth per minggu dihitung dari SELISIH nilai
// FOLLOWER (state), yaitu endFollowers minggu ini − endFollowers minggu lalu —
// BUKAN menjumlahkan diff_from_previous_day. Kenapa? Menjumlahkan selisih harian
// bisa tidak nyambung dengan kolom "Follower Akhir" (sumber beda: selisih vs nilai
// followers langsung, plus selisih hari pertama merujuk ke hari SEBELUM periode).
// Dengan pakai selisih nilai follower: net & follower akhir selalu konsisten, dan
// JUMLAH net mingguan = pertumbuhan bulanan (endFollowers akhir − followers awal).
//
// options.totalWeeks: kalau diisi, minggu tanpa data tetap muncul — netGrowth=0 dan
// endFollowers diwariskan dari minggu terakhir yang ada datanya (follower = keadaan,
// tak berubah di minggu tanpa data). Baseline (titik awal) = followers baris pertama
// periode, sama seperti followerGrowth, supaya total cocok.
...
export function weeklyFollowerTrend(historyRows = [], options = {}) {
  ...
  const baseline = Number(rows[0].followers); // followers awal periode (titik acuan)
  const endByWeek = new Map();
  for (const r of rows) endByWeek.set(weekOfMonth(dayOfMonth(r.date)), Number(r.followers)); // rows terurut -> jadi nilai TERAKHIR minggu itu

  const weeksPresent = [...endByWeek.keys()].sort((a, b) => a - b);
  // Tanpa totalWeeks: hanya minggu yang ada datanya (perilaku lama). Dengan totalWeeks:
  // 1..totalWeeks penuh (minggu kosong ikut tampil).
  const weekList = options.totalWeeks
    ? Array.from({ length: options.totalWeeks }, (_, i) => i + 1)
    : weeksPresent;
  ...
  for (const week of weekList) {
    if (endByWeek.has(week)) {
      const end = endByWeek.get(week);
      out.push({ week, label: `Minggu ${week}`, netGrowth: end - prevEnd, endFollowers: end });
      prevEnd = end;
      seenData = true;
    } else {
      // Minggu tanpa data: tidak ada perubahan; follower akhir diwariskan (atau null
      // kalau belum pernah ada data sama sekali sebelum minggu ini).
      out.push({ week, label: `Minggu ${week}`, netGrowth: 0, endFollowers: seenData ? prevEnd : null });
    }
  }
  return out;
}
