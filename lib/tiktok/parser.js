...
function parseContent(ws, options = {}) {
  const warnings = [];
  const reportISO = options.reportDate ? resolveReferenceISO({ snapshotDate: options.reportDate }) : null;
  const { headerRowNumber, resolved } = buildHeaderIndex(ws, {
    title: ALIASES.title, link: ALIASES.link, postdate: ALIASES.postdate,
  ...
  eachDataRow(ws, headerRowNumber, (row, rowNumber) => {
    ...
    const post = parseIndoDate(resolved.postdate ? cellValue(row.getCell(resolved.postdate)) : null, options);
