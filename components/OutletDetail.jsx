// File: components/OutletDetail.jsx
// Detail analitik SATU outlet (per-platform). Server Component dipakai bersama oleh
// route /dashboard/tiktok, /dashboard/instagram, /dashboard/threads — `defaultPlatform`
// menentukan tab Ringkasan Platform yang terbuka & judul halaman. Ringkasan lintas
// SEMUA outlet & platform ada terpisah di /dashboard (Umum).
...
// Bangun URL detail (base = route platform) dgn param relevan saja (kosong -> dibuang).
...
// Peta platform -> { basePath, tabIndex, judul } untuk tab & judul halaman.
const PLATFORM_META = {
  tiktok: { base: "/dashboard/tiktok", tab: 0, judul: "Detail TikTok", emoji: "🎵" },
  instagram: { base: "/dashboard/instagram", tab: 1, judul: "Detail Instagram", emoji: "📸" },
  threads: { base: "/dashboard/threads", tab: 2, judul: "Detail Threads", emoji: "🧵" },
};
      ...
      <div className="flex flex-wrap items-end justify-between gap-3 px-1">
        <div>
          <h1 className="flex items-center gap-2 text-xl font-bold tracking-tight text-ink sm:text-2xl">
            <span aria-hidden>{platformMeta.emoji}</span> Dashboard {defaultPlatform === "instagram" ? "Instagram" : defaultPlatform === "threads" ? "Threads" : "TikTok"}
          </h1>
          <p className="mt-0.5 text-sm" style={{ color: "var(--on-bg-soft)" }}>
            <Link href="/dashboard" className="font-semibold hover:underline" style={{ color: "var(--teal-900)" }}>← Ringkasan Umum</Link>
            <span className="mx-1.5">·</span>
            {selectedMonth ? `Meninjau ${labelBulan(selectedMonth)} — evaluasi kinerja tim` : "Detail per outlet (sepanjang masa)"}
          </p>
        </div>
        ...
        <>
          <div className="mt-2 flex flex-wrap items-center gap-3">
            ...
            <Link
              ...
              style={{ background: "#ffffff", color: "var(--teal-900)", border: "1px solid var(--line)" }}
            >
              📄 Laporan
            </Link>
            {branches.length > 1 && (
              <div className="flex flex-wrap gap-2">
                {branches.map((b) => (
                  <Link
                    key={b.id}
                    href={dashboardHref(basePath, { branch: b.id, cat: catFilter, month: selectedMonth })}
