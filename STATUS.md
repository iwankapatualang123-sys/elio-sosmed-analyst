...
- **Target per cabang & platform** (dipindah ke Pengaturan): tabel `tiktok_account_goals` kini PK komposit (cabang, platform: tiktok/instagram). Section "🎯 Target per Cabang & Platform" di Pengaturan (`GoalManager`, form per cabang × 2 platform, action `setAccountGoal`). **Pencapaian target** pindah ke tab **Ringkasan Platform** Dashboard (blok "🎯 Pencapaian Target sepanjang masa" di tab TikTok & Instagram, hanya bila target dipasang) — form edit lama di Dashboard dihapus.
