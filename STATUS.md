...
- **Target per BULAN × outlet × platform** (dikelola di Pengaturan): tabel `tiktok_account_goals` PK komposit (cabang, platform, target_month 'YYYY-MM'). Section "🎯 Target per Cabang & Platform" di Pengaturan (`GoalManager`: pemilih Bulan/Outlet/Platform + input target + Simpan, prefill otomatis dari kombinasi terpilih; action `setAccountGoal` upsert per kombinasi). **Pencapaian target BULANAN** di tab **Ringkasan Platform** Dashboard: capaian bulan yang ditinjau (ikut filter bulan; "Semua bulan" = bulan berjalan via `detail.thisMonth`) vs target bulan itu; label bulan di judul; muncul hanya bila target bulan itu dipasang. Form edit lama di Dashboard dihapus.
